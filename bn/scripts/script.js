...
async function displayPopularTools() {
    try {
        // For bn/index.html, we need to load bn/data/tools.json
        // The tools.json is always in the same directory as the current page's data folder
...
function fixLinkPaths() {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (target) {
            ...
            // Check if we're on a Bengali language page
            const isBengaliPage = window.location.pathname.includes('/bn/');
            ...
            if (isBengaliPage) {
                ...
                // Handle absolute root links that should be prefixed with /bn/
                if (href.startsWith('/') && !href.startsWith('/bn/')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/bn' + href;
                    window.location.href = newPath;
                }
                // Handle links like /list.html#text-tools that should become /bn/list.html#text-tools
                else if (href.startsWith('/list.html#') && !href.startsWith('/bn/list.html#')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/bn' + href;
                    window.location.href = newPath;
                }
