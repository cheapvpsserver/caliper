...
async function displayPopularTools() {
    try {
        // Determine directory depth of current page relative to es/ directory
...
function fixLinkPaths() {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (target) {
            ...
            // Check if we're on a Spanish language page
            ...
            if (isSpanishPage) {
                ...
                // Handle absolute root links that should be prefixed with /es/
                if (href.startsWith('/') && !href.startsWith('/es/')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/es' + href;
                    window.location.href = newPath;
                }
                // Handle links like /list.html#text-tools that should become /es/list.html#text-tools
                else if (href.startsWith('/list.html#') && !href.startsWith('/es/list.html#')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/es' + href;
                    window.location.href = newPath;
                }
