        ...
        .then(tools => {
            ...
            if (current) {
                ...
                if (related.length > 0) {
                    ...
                    container.innerHTML = '<p class="text-center text-gray-500">Aucun outil connexe trouvé.</p>';
                }
            } else {
                console.error('Current tool not found with slug:', currentSlug);
                const container = document.getElementById('relatedTools');
                container.innerHTML = '<p class="text-center text-gray-500">Échec du chargement des outils associés.</p>';
            }
        })
        .catch(error => {
