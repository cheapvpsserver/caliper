...
async function displayPopularTools() {
    try {
        // Determine directory depth of current page relative to ja/ directory
...
async function displayLatestTools() {
    try {
        ...
        console.error('最新ツールの読み込み中にエラーが発生しました:', error);
    }
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Determine the directory depth of the current page relative to the ja/ directory
    const pathSegments = window.location.pathname.split('/');
    const nonEmptySegments = pathSegments.filter(segment => segment !== '');
    
    // Remove language code (ja) from the path for relative path calculation
    const languageCode = 'ja';
    const languageIndex = nonEmptySegments.indexOf(languageCode);
    let relativeSegments = nonEmptySegments;
