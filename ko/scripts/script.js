...
async function displayPopularTools() {
    try {
        // Determine directory depth of current page relative to ko/ directory
...
async function displayLatestTools() {
    try {
        ...
        console.error('최신 도구 로딩 중 오류 발생:', error);
    }
}
...
async function loadHTML(url, element, position = 'beforeend') {
    try {
        ...
        console.error('HTML 로딩 중 오류 발생:', error);
    }
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Determine the directory depth of the current page relative to the ko/ directory
    const pathSegments = window.location.pathname.split('/');
    const nonEmptySegments = pathSegments.filter(segment => segment !== '');
    
    // Remove language code (ko) from the path for relative path calculation
    const languageCode = 'ko';
    const languageIndex = nonEmptySegments.indexOf(languageCode);
    let relativeSegments = nonEmptySegments;
...
function fixLinkPaths() {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (target) {
            ...
            // Check if we're on a Korean language page
            ...
            if (isKoreanPage) {
                ...
                // Handle absolute root links that should be prefixed with /ko/
                if (href.startsWith('/') && !href.startsWith('/ko/')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/ko' + href;
                    window.location.href = newPath;
                }
                // Handle links like /list.html#text-tools that should become /ko/list.html#text-tools
                else if (href.startsWith('/list.html#') && !href.startsWith('/ko/list.html#')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/ko' + href;
                    window.location.href = newPath;
                }
