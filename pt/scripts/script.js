...
async function displayPopularTools() {
    try {
        // Determine directory depth of current page relative to pt/ directory
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Determine the directory depth of the current page relative to the pt/ directory
    const pathSegments = window.location.pathname.split('/');
    const nonEmptySegments = pathSegments.filter(segment => segment !== '');
    
    // Remove language code (pt) from the path for relative path calculation
    const languageCode = 'pt';
    const languageIndex = nonEmptySegments.indexOf(languageCode);
    let relativeSegments = nonEmptySegments;
...
function fixLinkPaths() {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (target) {
            ...
            // Check if we're on a Portuguese language page
            const isPortuguesePage = window.location.pathname.includes('/pt/');
            ...
            if (isPortuguesePage) {
                ...
                // Handle absolute root links that should be prefixed with /pt/
                if (href.startsWith('/') && !href.startsWith('/pt/')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/pt' + href;
                    window.location.href = newPath;
                }
                // Handle links like /list.html#text-tools that should become /pt/list.html#text-tools
                else if (href.startsWith('/list.html#') && !href.startsWith('/pt/list.html#')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/pt' + href;
                    window.location.href = newPath;
                }
