...
async function displayPopularTools() {
    try {
        // Determine directory depth of current page relative to ru/ directory
        const pathSegments = window.location.pathname.split('/');
        const nonEmptySegments = pathSegments.filter(segment => segment !== '');
        
        // Remove language code (ru) from path for relative path calculation
...
async function displayLatestTools() {
    try {
        // Determine directory depth of current page relative to de/ directory
        const pathSegments = window.location.pathname.split('/');
        const nonEmptySegments = pathSegments.filter(segment => segment !== '');
        ...
        console.error('Ошибка при загрузке последних инструментов:', error);
    }
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Determine the directory depth of the current page relative to the ru/ directory
    const pathSegments = window.location.pathname.split('/');
    const nonEmptySegments = pathSegments.filter(segment => segment !== '');
    
    // Remove language code (ru) from the path for relative path calculation
    const languageCode = 'ru';
    const languageIndex = nonEmptySegments.indexOf(languageCode);
    let relativeSegments = nonEmptySegments;
...
function fixLinkPaths() {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (target) {
            ...
            // Check if we're on a Russian language page
            ...
            if (isRussianPage) {
                ...
                // Handle absolute root links that should be prefixed with /ru/
                if (href.startsWith('/') && !href.startsWith('/ru/')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/ru' + href;
                    window.location.href = newPath;
                }
                // Handle links like /list.html#text-tools that should become /ru/list.html#text-tools
                else if (href.startsWith('/list.html#') && !href.startsWith('/ru/list.html#')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/ru' + href;
                    window.location.href = newPath;
                }
