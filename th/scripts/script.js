...
async function displayPopularTools() {
    try {
        // Determine directory depth of current page relative to th/ directory
        const pathSegments = window.location.pathname.split('/');
        const nonEmptySegments = pathSegments.filter(segment => segment !== '');
        
        // Remove language code (th) from path for relative path calculation
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Determine the directory depth of the current page relative to the th/ directory
    const pathSegments = window.location.pathname.split('/');
    const nonEmptySegments = pathSegments.filter(segment => segment !== '');
    
    // Remove language code (th) from the path for relative path calculation
    const languageCode = 'th';
    const languageIndex = nonEmptySegments.indexOf(languageCode);
    let relativeSegments = nonEmptySegments;
...
function fixLinkPaths() {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (target) {
            ...
            // Check if we're on a Thai language page
            ...
            if (isThaiPage) {
                ...
                // Handle absolute root links that should be prefixed with /th/
                if (href.startsWith('/') && !href.startsWith('/th/')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/th' + href;
                    window.location.href = newPath;
                }
                // Handle links like /list.html#text-tools that should become /th/list.html#text-tools
                else if (href.startsWith('/list.html#') && !href.startsWith('/th/list.html#')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/th' + href;
                    window.location.href = newPath;
                }
