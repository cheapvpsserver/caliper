...
async function displayPopularTools() {
    try {
        // Determine directory depth of current page relative to ur/ directory
        const pathSegments = window.location.pathname.split('/');
        const nonEmptySegments = pathSegments.filter(segment => segment !== '');
        
        // Remove language code (ur) from path for relative path calculation
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const path = window.location.pathname;
    const langMatch = path.match(/^\/([a-z]{2})\//);
    const lang = langMatch ? langMatch[1] : 'ur';
    ...
    const isInLanguageDir = nonEmptySegments.length > 0 && nonEmptySegments[0].length === 2;
    ...
    if (nonEmptySegments.length > 0) {
        ...
        // Adjust directory depth for language directories
        if (isInLanguageDir) {
            directoryDepth -= 1;
        }
    }
    ...
    // Load header from the same language directory
    await loadHTML(`/${lang}/header.html`, document.body, 'prepend');
    
    // Load footer from the same language directory
...
function fixLinkPaths() {
    // Handle anchor links that should go to main list page
    ...
    document.addEventListener('click', function(event) {
        ...
        if (target) {
            ...
            // Check if we're on an Urdu language page
            const isUrduPage = window.location.pathname.includes('/ur/');
            ...
            if (isUrduPage) {
                ...
                // Handle absolute root links that should be prefixed with /ur/
                if (href.startsWith('/') && !href.startsWith('/ur/')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/ur' + href;
                    window.location.href = newPath;
                }
                // Handle links like /list.html#text-tools that should become /ur/list.html#text-tools
                else if (href.startsWith('/list.html#') && !href.startsWith('/ur/list.html#')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/ur' + href;
                    window.location.href = newPath;
                }
