        ...
        .then(tools => {
            ...
            if (current) {
                ...
                if (related.length > 0) {
                    ...
                    container.innerHTML = '<p class="text-center text-gray-500">Không tìm thấy công cụ liên quan.</p>';
                }
            } else {
                console.error('Current tool not found with slug:', currentSlug);
                const container = document.getElementById('relatedTools');
                container.innerHTML = '<p class="text-center text-gray-500">Không thể tải công cụ liên quan.</p>';
            }
        })
        .catch(error => {
