...
async function displayPopularTools() {
    try {
        // Determine directory depth of current page relative to vi/ directory
        const pathSegments = window.location.pathname.split('/');
        const nonEmptySegments = pathSegments.filter(segment => segment !== '');
        
        // Remove language code (vi) from path for relative path calculation
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Determine the directory depth of the current page relative to the vi/ directory
    const pathSegments = window.location.pathname.split('/');
    const nonEmptySegments = pathSegments.filter(segment => segment !== '');
...
function fixLinkPaths() {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (target) {
            ...
            // Check if we're on a Vietnamese language page
            const isVietnamesePage = window.location.pathname.includes('/vi/');
            ...
            if (isVietnamesePage) {
                ...
                // Handle absolute root links that should be prefixed with /vi/
                if (href.startsWith('/') && !href.startsWith('/vi/')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/vi' + href;
                    window.location.href = newPath;
                }
                // Handle links like /list.html#text-tools that should become /vi/list.html#text-tools
                else if (href.startsWith('/list.html#') && !href.startsWith('/vi/list.html#')) {
                    event.preventDefault();
                    const newPath = window.location.origin + '/vi' + href;
                    window.location.href = newPath;
                }
